...
vi.mock("@/lib/prisma", () => ({
  prisma: { bucketList: { count: vi.fn(), findMany: vi.fn() }, user: { findUnique: vi.fn() } },
}));
...
const mockCount = vi.mocked(prisma.bucketList.count);
...
function makeRequest(params: Record<string, string> = {}) {
  const url = new URL("http://localhost/api/dashboard/achievement-stats");
...
describe("GET /api/dashboard/achievement-stats", () => {
  ...
  it("username 없음 + 로그인 → 200, visibility 필터 없음(본인 데이터 전체)", async () => {
    ...
    const findManyWhere = mockFindMany.mock.calls[0][0]?.where as Record<string, unknown>;
    expect(countWhere).not.toHaveProperty("visibility");
    expect(findManyWhere).not.toHaveProperty("visibility");
  ...
  describe("username 파라미터 — 비소유자/익명 프로필 조회", () => {
    ...
    it("비로그인 + username → PUBLIC만 조회, areFriends 호출 안 함", async () => {
      ...
      expect(mockCount).toHaveBeenCalledWith({
        where: { userId: "target-1", visibility: { in: ["PUBLIC"] } },
      });
    ...
    it("로그인(친구) + username → PUBLIC+FRIENDS 조회", async () => {
      ...
      expect(res.status).toBe(200);
      expect(mockCount).toHaveBeenCalledWith({
        where: { userId: "target-1", visibility: { in: ["PUBLIC", "FRIENDS"] } },
      });
    });
